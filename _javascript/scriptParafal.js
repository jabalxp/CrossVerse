...
// Música de seleção (Nahyron decidiu fazer isso)
...
// Coloca o sonic como bola quando aperta a seta para baixo (Nahyron fez)
function spin() {
...
function mudarPersonagem() { // John Black fez, Parafal só deu aquela modelada e ajudada
    ...
    switch (selecao) {
        ...
        case "sonic":
            ...
            document.addEventListener("keydown", (event) => { // Nahyron fez
                switch (event.key) {
                    case "ArrowDown":
                        spin();
                        break;
...
function tiraVida() { // Parte inteira de tirar a vida - Matheus, Parte das Mortes - Parafal (Adicionou mais personagens) e João Pedro (fez o if dos bonecos mortos)
    telaMorte.style.display = "none";
    ...
    if (lifes >= 3) {
        ...
        const marioPosition = +window.getComputedStyle(mario).bottom.replace("px", ""); // Pega o valor CSS da propriedade bottom e remove o "px"

        pipe.style.animation = "none";
        pipe.style.left = `${pipePosition}px`; // esses dois blocos são pra travar o cano e o mario no lugar quando morrer
        mario.style.animation = "none";
        mario.style.bottom = `${marioPosition}px`;

        //Verifico qual o gif usado, baseado nisso coloco uma imagem de morte conforme o personagem que está sendo usado: João Pedro fez (Parafal só adicionou mais)
        if (mario.src.match("_media/gifs-principais/Pikachu.gif")) {
            ...
            mario.style.bottom = "-13px";
        } else if (mario.src.match("_media/gifs-principais/luffy.gif") || mario.src.match("_media/gifs-principais/luffyGear.gif")) { // Parafal 
...
function iniciarGame() {
    ...
    //Isso aqui verifica se o telaDisplay é "none" (se ele não está visível/aparecendo), se for ele ganha pontos e exibe no elemento com id "pontos"
        ...
        if (coin.style.display !== "none") {
            ...
            if (coinPosition <= 120 && coinPosition > 0 && marioPosition >= 120 && !coinCollected) {
                ...
                setTimeout(() => {
                    ...
                    coinCollected = false;
                }, 2000); // 2 (2000ms) segundos para reaparecer
            }
        }
    }, 100);
    ...
    const loop = setInterval(() => {
        ...
        if ( // If do Luffy Gear Five - Parafal
        ...
        if (pontos == 1500) { // Mathias fez (Parafal mexeu)
        ...
        if (pontos >= 95000) { // Nahyron e Matheus
        ...
        //Verifica se o "Mario" relou no cano se sim aparece a tela de morte e some com o cano
    ...
    document.addEventListener("keydown", (event) => { // Nahyron ajustou 
        switch (event.key) {
            case "ArrowUp":
                jump();
                break;
